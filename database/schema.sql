...
-- Architecture: Hexagonal with proper separation of concerns
-- Language: English with proper naming conventions
-- Integration: Supabase Auth (auth.users) + Custom business logic
-- ============================

-- Enable UUID extension for Supabase integration
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Note: Supabase manages auth.users table automatically
-- We extend it with user_profiles for business-specific data
...
CREATE TABLE IF NOT EXISTS companies (
    ...
    tax_id VARCHAR(20) UNIQUE, -- RUC/NIT/CUIT/etc.
...
CREATE TABLE IF NOT EXISTS positions (
    ...
    description TEXT,
    level VARCHAR(30), -- jr/sr/manager/director
    is_active BOOLEAN NOT NULL DEFAULT true,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS user_roles (
    ...
    role_id INTEGER NOT NULL REFERENCES roles(role_id) ON DELETE CASCADE,
    company_id INTEGER REFERENCES companies(company_id) ON DELETE CASCADE, -- role valid only in specific company
...
CREATE TABLE IF NOT EXISTS modules (
    ...
    code VARCHAR(80) NOT NULL UNIQUE, -- e.g., "SALES", "SALES.ORDERS"
...
CREATE TABLE IF NOT EXISTS functions (
    ...
    code VARCHAR(80) NOT NULL UNIQUE, -- e.g., "CREATE_ORDER", "APPROVE_PAYMENT"
    name VARCHAR(120) NOT NULL,
    description TEXT,
...
-- Use only when you need exceptions to role-based permissions
-- ============================
...
COMMENT ON TABLE persons IS 'Core identity information for all individuals in the system';
COMMENT ON TABLE user_profiles IS 'Business profile extension for Supabase auth.users';
COMMENT ON TABLE companies IS 'Organizations that employ persons and use the system';
COMMENT ON TABLE positions IS 'Job positions/titles within organizations';
COMMENT ON TABLE roles IS 'System roles that define sets of permissions';
COMMENT ON TABLE position_assignments IS 'Links persons to companies with specific positions';
COMMENT ON TABLE modules IS 'Hierarchical system modules and submodules';
COMMENT ON TABLE functions IS 'Specific functions/actions within modules';
COMMENT ON TABLE role_module_permissions IS 'CRUD permissions for roles at module level';
COMMENT ON TABLE role_function_permissions IS 'Granular permissions for roles at function level';

COMMENT ON COLUMN user_profiles.user_id IS 'Reference to Supabase auth.users(id) - authentication managed by Supabase';
COMMENT ON COLUMN user_profiles.preferences IS 'JSON object storing user preferences and settings';
COMMENT ON COLUMN position_assignments.is_primary IS 'Marks the main position for a person in a company';
COMMENT ON COLUMN modules.parent_id IS 'Self-reference: NULL = main module, NOT NULL = submodule';